...
class TastytradeTickBuffer:
    ...
    def warmup_with_historical_ticks(self, symbol, dataset, start, end, schema):
        ...
        try:
            ...
            if historical_bars and len(historical_bars) >= self.max_period:
                # Use real historical data
                self.processed_bars = historical_bars
                self.logger.info(f"Tastytrade warmup completed for {symbol}: {len(self.processed_bars)} real historical bars loaded")
            else:
                # Fallback to minimal bars if no historical data available
                self._create_initial_bars()
                self.logger.info(f"Tastytrade warmup completed for {symbol}: {len(self.processed_bars)} minimal bars created (no historical data)")
                
            self.historical_loaded = True
                
        except Exception as e:
            self.logger.error(f"Tastytrade warmup failed for {symbol}: {e}", exc_info=True)
            self._create_initial_bars()
            self.historical_loaded = True
        ...
        try:
            ...
            try:
                ...
                if current_spx_price:
                    ...
                    if bars:
                        self.logger.info(f"Created {len(bars)} SPX-based bars for SPXW warmup")
                        return bars
            except Exception as e:
    ...
    def _create_spx_based_bars(self, current_price, start, end):
        ...
        try:
            ...
            while current_time <= end_time:
                ...
                import random
                variation = random.uniform(-0.5, 0.5) * base_price * 0.001  # 0.1% variation
                ...
                close_price = open_price + random.uniform(-0.2, 0.2) * base_price * 0.001
    ...
    def _create_initial_bars(self):
        ...
        for i in range(self.max_period + 20):  # Extra bars for safety
    ...
    def start_live_subscription(self, symbol, dataset, schema, start_time=0):
        ...
        try:
            ...
            stream_thread, bar_thread = self.tastytrade_module.start_spxw_tick_streaming(
                symbol="SPXW",
                interval=1,
                logger=self.logger
            )
            ...
            if stream_thread and bar_thread:
                ...
                self.logger.error(f"Failed to start Tastytrade live subscription for {symbol}")
                
        except Exception as e:
            self.logger.error(f"Error starting Tastytrade live subscription for {symbol}: {e}", exc_info=True)
        ...
        with self.lock:
            ...
            try:
                ...
                if latest_bars and len(latest_bars) >= min_bars:
                    ...
                    self.logger.debug(f"Retrieved {len(df)} bars from Redis for {self.ticker}")
                    return df
